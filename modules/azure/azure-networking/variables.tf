...
variable "service_endpoints" {
  ...
  validation {
    condition     = alltrue([
                      ...
                      can(regex("^(Microsoft.AzureActiveDirectory|Microsoft.AzureCosmosDB|Microsoft.ContainerRegistry|Microsoft.EventHub|Microsoft.KeyVault|Microsoft.ServiceBus|Microsoft.Sql|Microsoft.Storage|Microsoft.Storage.Global|Microsoft.Web)$", service_endpoint))
                    ])
    error_message = "Each service endpoint must be a valid Azure service endpoint."
  }
  default = []
}
variable "tags" {
